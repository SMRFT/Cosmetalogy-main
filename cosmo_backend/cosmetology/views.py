...
def get_summary_by_interval(request, interval):
    ...
    summaries = SummaryDetail.objects.filter(appointmentDate__gte=start_date, appointmentDate__lte=end_date)
    serializer = SummaryDetailSerializer(summaries, many=True)
...
def update_billing_data(request):
    ...
    new_data = request.data.get('table_data')
    if not patientUID or not date or not new_data:
        return Response({'error': 'Invalid data'}, status=status.HTTP_400_BAD_REQUEST)

    # Find and update the record
    result = collection.find_one_and_update(
        {'patientUID': patientUID, 'appointmentDate': date},
        {'$set': {'table_data': new_data}},
...
def delete_billing_data(request):
    ...
    if request.method == 'DELETE':
        try:
            ...
            BillingData.objects.filter(id=record_id).delete()
